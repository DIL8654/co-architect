...
public sealed class MultiAgentArchitectureAnalysisService : IMultiAgentArchitectureAnalysisService
{
    ...
    public async Task<AgentAnalysisResult> AnalyzeAsync(ArchitectureDiagram diagram, CancellationToken cancellationToken)
    {
        ...
        var plannerSummary = BuildPlannerSummary(diagram, facts, frameworkDecision);
        traces.Add(CreateTrace(
            "Planner",
            "Build the architecture review plan and choose specialist agents.",
            plannerStartedAt,
            plannerSummary,
            new[]
            {
                $"Selected frameworks: {string.Join(", ", selectedFrameworks)}",
                $"Framework source: {frameworkDecision.Source}",
        ...
        var expertBaseline = await _expertAgentService.AnalyzeAsync(
            diagram.Id,
            BuildExpertPrompt(diagram, facts, selectedFrameworks, effectiveWeights),
            cancellationToken);
        traces.Add(CreateTrace(
            "Foundry Expert",
        ...
        foreach (var framework in selectedFrameworks)
        {
            ...
            var output = BuildFrameworkSpecialistOutput(framework, facts, diagram);
        ...
        var tradeoffOutput = BuildTradeoffOutput(diagram, facts, specialistOutputs, expertBaseline);
        traces.Add(CreateTrace(
            "Trade-off Balancer",
        ...
        var combined = CombineResults(diagram.Id, expertBaseline, specialistOutputs, tradeoffOutput);

        var criticStartedAt = DateTime.UtcNow;
        var criticNotes = CriticReview(combined);
        traces.Add(CreateTrace(
            "Critic",
            "Verify clarity, remove duplicates, and tighten the final report.",
            criticStartedAt,
            "Validated duplicates, recommendation coverage, and evidence quality before returning the final response.",
            criticNotes.Take(3)));
        ...
        return new AgentAnalysisResult
        {
            ...
            OpenQuestions = BuildOpenQuestions(diagram, facts),
    ...
    {
        var highlights = new List<string>();
        var tradeoffs = new List<Tradeoff>(expertBaseline.Tradeoffs);
    ...
    {
        var evidence = new List<EvidenceItem>(expertBaseline.Evidence);
        var missingControls = new List<MissingControl>(expertBaseline.MissingControls);
        var recommendations = new List<Recommendation>(expertBaseline.Recommendations);
